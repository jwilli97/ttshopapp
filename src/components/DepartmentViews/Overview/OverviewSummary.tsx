...
export function OverviewSummary({ todayOrderCount, pendingOrderCount, inProgressOrderCount, outForDeliveryOrderCount, readyForPickupOrderCount, completedOrderCount, cancelledOrderCount }: OrdersSummaryProps) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <SummaryCard title="Pending Orders" value={pendingOrderCount} description="Number of orders currently being processed" icon={<Package className="h-4 w-4" />} />
            <SummaryCard title="In Progress" value={inProgressOrderCount} description="Number of orders ready for dispatch" icon={<ArrowUp className="h-4 w-4" />} />
